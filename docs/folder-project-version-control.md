...
- 面向普通流程的 `IFolderProjectHistoryService` 只提供状态、初始化、创建/删除还原点、历史、还原点内容、异常恢复和放弃未记录修改；不提供身份、分支、暂存区、储藏、合并或远端能力。
...
| 生成 Pack | 工程磁盘快照 → 工程外 `.pack`；有未记录修改时先询问是否仍要生成；《全面战争：战锤 3》的 PFH5 可压缩资源使用 ZSTD，其他游戏不压缩 | 不保存内存、不创建还原点、不生成备份、不清除资源树修改标记 |
| 刷新工程历史 | 读取磁盘全部未记录修改和最近还原点 | 不改写磁盘或创建还原点 |
| 创建还原点 | 先对账工程磁盘和空目录配置，再记录所有磁盘修改 | 不只记录底层 index 的局部选择；无修改时不创建重复还原点 |
| 删除还原点 | 删除所选非初始还原点；保持工程磁盘文件不变，并重写当前历史线以保留后续还原点的相同文件快照 | 不删除初始还原点、不改动额外历史线；删除当前还原点时，其差异转为未记录修改 |
| 查看还原点内容 | 首次读取该还原点相对上一个还原点的文件变化并持久缓存，之后直接复用 | 不恢复或改写当前工程 |
| 恢复整个工程 | 有未记录修改时先创建安全还原点，再把所选还原点的完整内容作为新的当前还原点；有未记录修改时也允许恢复当前还原点 | 不移动、不删除或重写旧还原点 |
...
恢复整个工程、放弃全部未记录修改和恢复异常工程历史属于批量磁盘操作，必须通过 `FolderProjectGitOperationCoordinator` 关闭相关编辑器、卸载容器、执行核心事务并重新加载。单文件恢复和放弃选中修改不卸载整个工程：先关闭受影响的编辑页，有未保存内容时允许取消；写入后重新对账容器并发布文件树变化，再以原编辑器类型重新打开仍存在的文件。重新打开失败也必须回滚磁盘并重新加载操作前的文件版本。这些操作都只在底层元数据目录保留本次操作所需的临时回滚状态；磁盘与界面刷新全部成功后立即清理。若写入、删除、历史更新或重新加载失败，必须先恢复操作前磁盘、还原点历史和底层 index，再重新加载原工程状态。
...
初始化会把工程当时的完整内容记录为本地 Git 持久基线；这个基线保存在工程自身的 `.git` 中，关闭 AE 或重启电脑后仍然有效。此后打开工程、工程历史刷新、资源树变化标记、退出保护、创建还原点、恢复、放弃修改和异常恢复都先相对该基线取得新增、修改、删除、重命名等差异。普通显示状态不额外打开资源；需要安全校验的流程也只打开本次新增、修改或重命名后仍存在的文件，删除项只检查路径，不再递归枚举或重新读取未变化资源。安全校验按真实差异清单报告路径与已完成/总数，不估算百分比。

每个还原点的内容摘要和完整变化清单按不可变提交 ID 缓存在工程自身的 `.git/asseteditor-cn/history-cache-v1` 中，不进入资源树或还原点历史。初始化和新建还原点会立即写入完整缓存；旧工程中尚未缓存的历史只在第一次查看时计算一次。关闭 AE、重启电脑或切换界面后继续复用缓存；缺失、损坏或版本不兼容的缓存只会触发重新计算，不能改变或阻止读取真实还原点。首次计算只做完全相同内容的重命名识别，变化类型按路径格式分类而不读取资源内容，避免对大型二进制资源执行内容相似度或逐文件内容扫描；详情进度按真实完成数节流更新并保证最终完成数等于总数。

这项增量检查有一个明确边界：未发生内容变化、但在 AE 关闭期间单独改变了文件权限的资源，不会为了提前发现权限问题而在每次打开或退出时被重新读取；当后续操作实际读取或写入该资源时，现有路径和异常门禁仍会安全阻止操作并提示错误。用这一边界换取大型工程正常操作不再反复扫描数万份未修改资源。
...
- 主窗口的工程历史面板打开时快速读取未记录状态；文件变化事件只把刷新排入当前工程，历史操作进行中时等待完成后再刷新。需要改写磁盘或关闭工程的安全流程继续校验当前差异文件，但不重读持久基线中未变化的资源。
- 历史按批加载，可继续读取更早还原点；普通刷新保留已经加载的范围、仍然有效的还原点和文件选择，不清空未变化的列表。切换到其他工程时重置加载范围和选择，旧工程的异步读取结果不能应用到新工程。
