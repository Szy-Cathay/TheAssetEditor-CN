...
顶部工具栏保留常用操作及专业工具图标，模型处理、骨骼与蒙皮、选区与顶点工具随选择模式显示，照片工作室和快捷键说明始终提供图标入口；中文菜单作为辅助入口。视口统计信息默认收起，可在“渲染”菜单显示或隐藏；此开关只影响当前 Kitbash 视口，不修改模型或其他编辑器。场景树固定节点和保存策略的中文名称仅用于显示，内部节点名称和策略标识保持不变；新建用户编组使用中文默认名称，已有名称保持原样；界面中的 LOD 保留通用缩写。操作完成、撤销和重做提示由 Kitbash 的 WPF 覆盖层在视口底部居中显示，使用中文及主题颜色，只保留最新一条并在 3 秒后隐藏；不捕获鼠标，离开或关闭编辑器后清除。共享命令事件提供命令类型与重做标记，Kitbash 不再绘制旧的左上角命令文字。统计文字的位置需避开可收起的左侧工具栏；编组和隔离菜单应显示当前状态对应的下一步操作。

左侧选择与变换工具栏透明叠放在视口左上角，不为工具栏预留贯穿视口的列。箭头收起时只隐藏工具图标，保留当前选择、变换工具和快捷键；展开恢复原状态。图标按当前视口背景从主题文字色与画布色中选择对比度较高的一种，悬停和选中时沿用按钮主题。收起与展开不改变视口尺寸或模型状态。
...
- 自发光渐变的预览按遮罩值在 0～1 之间采样；流动、扭曲和脉冲参数可保存，但当前渲染器未实现动态预览，界面应明确区分；
- 骨骼与附着点表格只读；无额外附着时显示“无”，初始化和重新显示默认项不得修改模型；
...
材质复制使用单一来源选择窗口；搜索按普通文字匹配模型、LOD 和贴图路径，取消不应视为错误。Photo Studio 界面的相机角度以度显示，底层相机和已有设置文件继续使用弧度。顶点诊断的统计结果必须在窗口内可见，不能只写日志。

子窗口关闭、编辑器关闭或作用域释放时必须解除事件、释放 GameComponent/GPU 资源并停止持有主场景。不要把子工具注册成跨编辑器单例。

专业工具的历史必须保留处理前后的节点、几何和材质实例；重做恢复同一结果，保证后续变换和属性编辑仍指向场景中的对象。拆分选中面时保留未选中的部分。合并需转换到结果网格的坐标空间，并在修改场景前拒绝超过 65536 个顶点的结果。减面比例以三角面数为基准，LOD 生成遵循同一含义。

动画预览中只合并动画源和世界变换一致的蒙皮网格，避免把对象偏移错误地烘焙到骨骼空间；不兼容的输入保留独立网格。

Mesh Fitter 调参时使用实时预览，确认只写入一次，取消恢复原动画播放器。Pin 的源顶点必须来自带骨骼权重的网格；Skin Wrap 在共同世界空间中查找邻近表面，按输出格式可保存的权重槽数筛选并归一化。Vertex Debugger 的定位框和方向线跟随当前显示姿态，表格继续显示原始顶点数据。
