...
namespace Shared.GameFormats.Wwise.Wem.V132
{
    ...
    {
        public const string ChunkTag = "akd ";

        private const uint MinimumPayloadSize = 16;
        ...
        {
            if (chunk.BytesLeft < MinimumPayloadSize)
                throw new InvalidDataException($"WEM akd chunk must be at least {MinimumPayloadSize} bytes, got {chunk.BytesLeft}.");
        ...
        public override byte[] WriteData()
        {
            ...
            var totalSize = MinimumPayloadSize + (uint)EnvelopeData.Length;
        ...
        {
            if (EnvelopeData.Length % EnvelopePointSize != 0)
                throw new InvalidDataException($"Envelope data length must be a multiple of {EnvelopePointSize} bytes.");

            var derivedEnvelopePointCount = (uint)(EnvelopeData.Length / EnvelopePointSize);
            if (HdrEnvelopePointCount != 0 && HdrEnvelopePointCount != derivedEnvelopePointCount)
                throw new InvalidDataException($"Envelope point count mismatch. Expected {HdrEnvelopePointCount} points but got {derivedEnvelopePointCount}.");

            return (int)(MinimumPayloadSize + (uint)EnvelopeData.Length);
        }
    }
}
