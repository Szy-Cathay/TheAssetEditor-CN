...
namespace Shared.GameFormats.Video
{
    public class WebMFile
    {
        ...
        private byte[] BuildInfo()
        {
            ...
            var videoDurationMs =
                FrameTable.Count / (double)Framerate * 1000.0;
            var audioDurationMs = VorbisAudioPackets.Count == 0
                ? 0
                : VorbisAudioPackets[^1].TimestampMilliseconds;
            var durationMs = Math.Max(videoDurationMs, audioDurationMs);
            var timestampScaleNanoseconds = 1_000_000UL;

            var timestampScale = UintElement(idTimestampScale, timestampScaleNanoseconds);
            var muxingApp = StringElement(idMuxingApp, "AssetEditor.CN");
            var writingApp = StringElement(idWritingApp, "AssetEditor.CN");
        ...
        private void WriteClusters(BinaryWriter writer)
        {
            ...
            while (videoIndex < FrameTable.Count ||
                   audioIndex < VorbisAudioPackets.Count)
            {
                using var clusterContent = new MemoryStream();
                using var clusterWriter = new BinaryWriter(clusterContent);

                var nextVideoTimestampMs =
                    videoIndex < FrameTable.Count
                        ? (long)(videoIndex * msPerVideoFrame)
                        : long.MaxValue;
                var nextAudioTimestampMs =
                    audioIndex < VorbisAudioPackets.Count
                        ? VorbisAudioPackets[audioIndex]
                            .TimestampMilliseconds
                        : long.MaxValue;
                var clusterTimestampMs = Math.Min(
                    nextVideoTimestampMs,
                    nextAudioTimestampMs);
                clusterWriter.Write(
                    UintElement(
                        idTimestamp,
                        checked((ulong)clusterTimestampMs)));
                       ...
                       audioIndex < VorbisAudioPackets.Count)
                {
                    ...
                    nextAudioTimestampMs =
                        audioIndex < VorbisAudioPackets.Count
                            ...
                            : long.MaxValue;
                    var writeVideo =
                        nextVideoTimestampMs <= nextAudioTimestampMs;
                    var blockTimestampMs = writeVideo
                        ? nextVideoTimestampMs
                        : nextAudioTimestampMs;
                    var relativeMs =
                        blockTimestampMs - clusterTimestampMs;

                    var isKeyFrame =
                        writeVideo &&
                        FrameTable[videoIndex].IsKeyFrame;
                    var exceedsSimpleBlockTimestampRange =
                        relativeMs > short.MaxValue;
                    var reachedTargetClusterDuration =
                        isKeyFrame &&
                        relativeMs >= maxClusterDurationMs;
                    if (blocks.Count > 0 &&
                        (exceedsSimpleBlockTimestampRange ||
                         reachedTargetClusterDuration))
                    {
                        break;
                    }

                    var relativeTimestampMs = checked((short)relativeMs);
                    if (writeVideo)
                    {
                        ...
                        blocks.Add(
                            new ClusterBlock(
                                ...
                                BuildSimpleBlock(
                                    ...
                                    relativeTimestampMs,
                                    isKeyFrame,
                                    trackNumber: (int)TrackNumberVideo)));
                        videoIndex++;
                    }
                    else
                    {
                        ...
                        blocks.Add(
                            new ClusterBlock(
                                ...
                                BuildSimpleBlock(
                                    ...
                                    relativeTimestampMs,
                                    isKeyFrame: false,
                                    trackNumber: (int)TrackNumberAudio)));
                        audioIndex++;
                    }
                }
        ...
        {
            if (Width == 0 || Height == 0)
                throw new InvalidDataException("WebM video dimensions must be greater than zero.");
            if (Framerate <= 0 || float.IsNaN(Framerate) || float.IsInfinity(Framerate))
                throw new InvalidDataException("WebM video framerate must be finite and greater than zero.");
            if (FrameTable.Count == 0)
                throw new InvalidDataException("WebM video must contain at least one frame.");
            ...
            for (var frameIndex = 0; frameIndex < FrameTable.Count; frameIndex++)
            {
                ...
                if (frame.Size == 0)
                    throw new InvalidDataException($"WebM frame {frameIndex} has a size of zero.");
                if (frame.Size > int.MaxValue)
                    throw new InvalidDataException($"WebM frame {frameIndex} is too large.");
                if (frame.Offset != expectedOffset)
                    throw new InvalidDataException($"WebM frame {frameIndex} has a non-canonical data offset.");

                expectedOffset = checked(expectedOffset + frame.Size);
                if (expectedOffset > FrameData.Length)
                    throw new InvalidDataException($"WebM frame {frameIndex} extends beyond the available frame data.");
            }

            if (expectedOffset != FrameData.Length)
                throw new InvalidDataException("WebM frame table does not consume all available frame data.");

            var hasCodecPrivate = VorbisCodecPrivate is { Length: > 0 };
            var hasAudioPackets = VorbisAudioPackets.Count > 0;
            if (hasCodecPrivate != hasAudioPackets)
                throw new InvalidDataException("WebM Vorbis track data is incomplete.");
            if (!hasCodecPrivate)
                return;

            if (VorbisSampleRate <= 0 || VorbisChannels <= 0)
                throw new InvalidDataException("WebM Vorbis track format is invalid.");

            long previousTimestamp = -1;
            for (var packetIndex = 0; packetIndex < VorbisAudioPackets.Count; packetIndex++)
            {
                var packet = VorbisAudioPackets[packetIndex];
                if (packet.Data.Length == 0)
                    throw new InvalidDataException($"WebM Vorbis packet {packetIndex} is empty.");
                if (packet.TimestampMilliseconds < previousTimestamp)
                    throw new InvalidDataException("WebM Vorbis packet timestamps must be ordered.");

                previousTimestamp = packet.TimestampMilliseconds;
            }
        }
    }
}
