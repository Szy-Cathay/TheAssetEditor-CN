...
namespace GameWorld.Core.Rendering.Geometry
{
    public class PrimitiveConstructor
    {
            ...
            float size = 1f)
        {
            ...
            if (float.IsFinite(size) == false || size <= 0)
        ...
        public MeshObject CreateSphere(
            UiVertexFormat vertexFormat,
            string skeletonName,
            int resolution = 10,
            float radius = 0.5f)
        {
            ...
            if (resolution > 180)
                throw new ArgumentOutOfRangeException(nameof(resolution));
            if (float.IsFinite(radius) == false || radius <= 0)
            ...
            for (var stack = 0; stack <= stacks; stack++)
            {
                ...
                for (var slice = 0; slice <= slices; slice++)
                {
                    ...
                    var tangent = new Vector3(-MathF.Sin(theta), 0, MathF.Cos(theta));
        ...
        private static void AssignGeometry(
            MeshObject mesh,
            List<VertexPositionNormalTextureCustom> vertices,
            List<ushort> indices)
        {
